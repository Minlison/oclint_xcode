...
static llvm::cl::list<std::string> argRulesPath("R",
    ...
    llvm::cl::ZeroOrMore,
    llvm::cl::cat(OCLintOptionCategory));
static llvm::cl::list<std::string> argReportersPath("RT",
    llvm::cl::Prefix,
    llvm::cl::desc("Add directory to reporters loading path"),
    llvm::cl::value_desc("directory"),
    llvm::cl::ZeroOrMore,
    llvm::cl::cat(OCLintOptionCategory));
...
std::vector<std::string> oclint::option::reporterPath()
{
    if (argReportersPath.size() > 0)
    {
        return argReportersPath;
    }
    std::string defaultRulePath = libPath() + "/oclint/reporters";
    std::vector<std::string> defaultRulesPath { defaultRulePath };
    return defaultRulesPath;
}
